...
PLUGIN_PARAMS	g_PluginParams = {200, 1 * 100, 100, 30};
HINSTANCE g_hInst;
//MAIN()
...
void	Run(PCSTR psName, int srcNum, const GimpParam *pSrc, int *pDstNum, GimpParam **ppDst)
{
	...
	PFNFUNC pfnGimpSetData = libGimp.GetFunction("gimp_set_data");
	PFNFUNC pfnGimpGetData = libGimp.GetFunction("gimp_get_data");
...
//		double ofs = offset + ::fmod(::rand(), variation) - variation * 0.5;
		...
		//(*pfnGimpProgressUpdate)((double)i / (double)iMax);
	}
