...
void Proc(Dll &libGimp, GimpDrawable *pDrawable) {
	...
	gimp_pixel_rgn_get_rect(&rgnSrc, &img[0], x1, y1, w, h);
	Apply(img, w, h, g_PluginParams.dx1, g_PluginParams.dy1, g_PluginParams.dx2, g_PluginParams.dy2);

	gimp_pixel_rgn_set_rect(&rgnDst, &img[0], x1, y1, w, h);
